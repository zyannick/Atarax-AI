...
async def check_manifest(orch: AtaraxAIOrchestrator = Depends(get_unlocked_orchestrator)):  # type: ignore
    is_valid = await orch.rag.check_manifest_validity()
    if is_valid:
...
async def health_check(orch: AtaraxAIOrchestrator = Depends(get_unlocked_orchestrator)):  # type: ignore
    is_healthy = await orch.rag.health_check()
...
async def add_directory(
    ...
    """
    task_routine = orch.rag.add_watch_directories(request.directories)
    future = await req_manager.submit_request(
        coro=task_routine, priority=RequestPriority.HIGH
    )
    task_id = task_manager.create_task(future)
