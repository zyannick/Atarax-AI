...
class ChainRunner:
    def __init__(
        self,
        chain_task_manager: ChainTaskManager,
        ...
        self.chain_task_manager = chain_task_manager
    ...
    async def run_chain(
        ...
        for i, step in enumerate(chain_definition):
            ...
            task = self.chain_task_manager.get_task(task_id)
            ...
            self.logger.info(f"Running Step {i}: Task '{task_id}'")
            ...
            try:
                ...
                self.logger.info(
                    f"--- Step {i} Successful. Result: {str(final_result)}... ---"
                )

            except Exception as e:
                self.logger.error(f"--- Error in Step {i}, Task '{task_id}' ---")
