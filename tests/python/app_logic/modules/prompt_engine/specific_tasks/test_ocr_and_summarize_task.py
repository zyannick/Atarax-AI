...
from ataraxai.app_logic.modules.prompt_engine.specific_tasks.ocr_and_summarize_task import OCRandSummarizeTask
...
def test_load_resources_success(monkeypatch, ocr_task):
    ...
    ocr_task._load_resources()  # Should not raise

def test_load_resources_tesseract_not_found(monkeypatch, ocr_task):
    class DummyError(Exception): pass
    monkeypatch.setattr("pytesseract.get_tesseract_version", lambda: (_ for _ in ()).throw(getattr(__import__('pytesseract'), 'TesseractNotFoundError', Exception)("not found")))
    with pytest.raises(Exception):
        ocr_task._load_resources()

def test_execute_raises_on_missing_image_path(ocr_task):
    with pytest.raises(ValueError):
        ocr_task.execute({}, mock.Mock(), mock.Mock(), mock.Mock(), {})

@mock.patch("ataraxai.app_logic.modules.prompt_engine.specific_tasks.ocr_and_summarize_task.Image")
@mock.patch("ataraxai.app_logic.modules.prompt_engine.specific_tasks.ocr_and_summarize_task.pytesseract")
...
def test_execute_successful_flow(mock_pytesseract, mock_image, ocr_task):
    ...
    result = ocr_task.execute(
        ...
        mock.Mock(),
        mock_prompt_manager,
        mock_core_ai_service,
        {"temperature": 0.5}
    )
    assert result == "Summary."
    mock_prompt_manager.load_template.assert_called_once_with("ocr_summarize")
    mock_core_ai_service.process_prompt.assert_called_once()
