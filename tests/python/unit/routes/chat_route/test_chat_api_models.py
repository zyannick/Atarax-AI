...
def test_delete_project_response_fields():
    pid = uuid.uuid4()
    resp = DeleteProjectResponseAPI(
        project_id=pid, name="Proj", description="Desc", status=Status.DELETED
    )
    assert resp.project_id == pid
    assert resp.name == "Proj"
    assert resp.description == "Desc"
    assert resp.status == Status.DELETED


def test_session_response_fields():
    sid = uuid.uuid4()
    pid = uuid.uuid4()
    resp = SessionResponseAPI(
        status=Status.SUCCESS,
        session_id=sid,
        title="Title",
        project_id=pid,
...
def test_delete_session_response_fields():
    sid = uuid.uuid4()
    pid = uuid.uuid4()
    resp = DeleteSessionResponseAPI(
        session_id=sid, title="Title", project_id=pid, status=Status.DELETED
    )
    assert resp.session_id == sid
    assert resp.title == "Title"
    assert resp.project_id == pid
