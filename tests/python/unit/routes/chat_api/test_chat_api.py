...
#     mock_orchestrator._state = AppState.UNLOCKED
#     yield client

@pytest.fixture
def unlocked_client(client):
    mock_orchestrator = client.app.state.orchestrator
    mock_orchestrator.get_state = mock.AsyncMock(return_value=AppState.UNLOCKED)
...
def locked_client(client):
    mock_orchestrator = client.app.state.orchestrator
    mock_orchestrator._state = AppState.LOCKED
    yield client
...
def first_launch_client(client):
    mock_orchestrator = client.app.state.orchestrator
    mock_orchestrator._state = AppState.FIRST_LAUNCH
    yield client



@pytest.mark.asyncio
async def test_create_new_project_success(unlocked_client):
    mock_project_result = mock.MagicMock()
    mock_project_result.id = uuid.uuid4()
    mock_project_result.name = "Test Project"
    mock_project_result.description = "A test project"

    mock_chat_manager = mock.MagicMock()
    mock_chat_manager.create_project = mock.AsyncMock(return_value=mock_project_result)

    mock_orchestrator = unlocked_client.app.state.orchestrator
    mock_orchestrator.get_chat_manager = mock.AsyncMock(return_value=mock_chat_manager)

    mock_req_manager = unlocked_client.app.state.request_manager
    mock_req_manager.submit_request = mock.AsyncMock(return_value=mock_project_result)

    payload = {"name": "Test Project", "description": "A test project"}
    response = unlocked_client.post("/api/v1/chat/projects", json=payload)

    assert response.status_code == status.HTTP_200_OK, f"API error: {response.text}"
    data = response.json()
    assert data["project_id"] == str(mock_project_result.id)
    assert data["name"] == "Test Project"



@pytest.mark.asyncio
async def test_create_new_project_failure(unlocked_client):
    mock_req_manager = unlocked_client.app.state.request_manager

    mock_req_manager.submit_request = mock.AsyncMock(side_effect=Exception("DB Error"))
    
    mock_orchestrator = unlocked_client.app.state.orchestrator
    mock_orchestrator.get_chat_manager = mock.AsyncMock()
...
# @pytest.mark.asyncio
# async def test_delete_project_success(unlocked_client):

#     mock_request_manager = unlocked_client.app.state.request_manager
#     mock_request_manager.submit_request = mock.AsyncMock()


#     mock_orchestrator = unlocked_client.app.state.orchestrator
#     mock_orchestrator.chat.get_project = mock.AsyncMock()
#     mock_orchestrator.chat.delete_project = mock.AsyncMock(return_value=True)
