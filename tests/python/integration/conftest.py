...
def logger(app_directories: AppDirectories) -> Generator[logging.Logger, None, None]:
    ...
    """
    logger = AtaraxAILogger(log_dir=str(app_directories.logs)).get_logger()
    yield logger
...
def integration_client(orchestrator: AtaraxAIOrchestrator):
    """
    Master fixture for INTEGRATION TESTS. Provides a TestClient with a REAL,
    fully assembled orchestrator that uses temporary directories.
    """
    app.dependency_overrides[AtaraxAIOrchestratorFactory.create_orchestrator] = lambda: orchestrator
